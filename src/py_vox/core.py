...
def report(format: Literal['json', 'markdown'] = 'json', output_dir: str = '.py_vox_logs') -> str:
    ...
    if format == 'markdown':
        ...
        file_path = dir_path / f"py_vox_report_{timestamp}.md"
    else:
        content = format_json(events)
        file_path = dir_path / f"py_vox_report_{timestamp}.json"
